...
export default function BenchmarkingSucursales({ rows }: Props) {
  const benchmarkRows = useMemo<BranchRow[]>(() => {
    ...
    for (const row of rows) {
      ...
      branchSales.set(
        sucursal,
        (branchSales.get(sucursal) ?? 0) + venta
      );

      branchUnits.set(
        sucursal,
        (branchUnits.get(sucursal) ?? 0) + cantidad
      );

      const productMap =
        branchProducts.get(sucursal) ??
        new Map<string, number>();

      productMap.set(
        producto,
        (productMap.get(producto) ?? 0) + venta
      );
    ...
    const participacionDisponible =
      totalSales > 0 &&
      !hasNegativeBranchSales;

    return [...branchSales.entries()]
      .map(([sucursal, ventas]) => {
        const units =
          branchUnits.get(sucursal) ?? 0;
              ...
              {benchmarkRows.map(
                (row, index) => (
                  <tr key={row.sucursal}>
                    ...
                    <td style={styles.td}>
                      <div
                        style={styles.shareCell}
                      >
                        <div
                          style={styles.shareTop}
                        >
                          <span
                            style={
                              styles.sharePct
...
> = {
  wrapper: {
    ...
    padding: "12px 14px",
    border:
      "1px solid rgba(109,126,219,0.16)",
    boxShadow:
      "0 10px 24px rgba(46,13,79,0.04)",
  },
  ...
  rankFirst: {
    ...
    fontWeight: 850,
    boxShadow:
      "0 8px 18px rgba(89,101,195,0.18)",
  },
